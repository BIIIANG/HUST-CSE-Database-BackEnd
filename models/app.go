...
func InfoGradeQueryDB(infoGradeQuery *InfoGradeQuery, student *Student, scs *[]SC, scRows *int64) (error, int64) {
	tx := dao.DB.Model(&Student{}).Where("sno = ?", infoGradeQuery.Sno).Take(&student)
	if tx.Error == nil {
		// dao.DB.Model(&SC{}).Where("sno = ?", infoGradeQuery.Sno).Count(scRows)
		if infoGradeQuery.PageNum > 0 && infoGradeQuery.PageSize > 0 {
			tx = dao.DB.Model(&SC{}).Where("sno = ?", infoGradeQuery.Sno).Count(scRows).Limit(infoGradeQuery.PageSize).Offset((infoGradeQuery.PageNum - 1) * infoGradeQuery.PageSize).Order("cno").Find(&scs)
		}
	}
...
func DeptRankQueryDB(dept string, deptRank *[]DeptRank, rows *int64) (error, int64) {
	tx := dao.DB.Model(&Course{}).Raw("SELECT gorm_sc.sno, sname, gorm_sc.cno, cname, grade FROM gorm_student, gorm_course, gorm_sc WHERE gorm_student.sno=gorm_sc.sno AND gorm_sc.cno=gorm_course.cno AND gorm_student.sdept = ? ORDER BY grade DESC;", dept).Scan(&deptRank)
	*rows = tx.RowsAffected
	return tx.Error, tx.RowsAffected
}

func DeptInfoQueryDB(deptInfo *[]DeptInfo, rows *int64) (error, int64) {
	tx := dao.DB.Model(&Course{}).Raw("SELECT Sdept Dept, AVG(Grade) AvgGrade, MAX(Grade) MaxGrade, MIN(Grade) MinGrade, " +
