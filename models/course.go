...
func CourseQueryDB(couQuery *CourseQuery, courses *[]Course, rows *int64) (error, int64) {
	if couQuery.Filter == "true" {
		tx := dao.DB.Model(&Course{}).Raw("SELECT * FROM gorm_course WHERE NOT EXISTS (SELECT * FROM gorm_sc WHERE gorm_course.cno = gorm_sc.cno);").Scan(&courses)
		*rows = tx.RowsAffected
		return tx.Error, tx.RowsAffected
	} else {
		dao.DB.Model(&Course{}).Count(rows)
		if couQuery.PageNum > 0 && couQuery.PageSize > 0 {
			tx := dao.DB.Limit(couQuery.PageSize).Offset((couQuery.PageNum - 1) * couQuery.PageSize).Order("cno").Find(&courses)
			return tx.Error, tx.RowsAffected
		} else {
			return nil, 0
		}
...
func CourseInsertDB(newCou *Course) (error, int64) {
	tx := dao.DB.Create(&newCou)
	return tx.Error, tx.RowsAffected
}

func CourseUpdateDB(newCou *Course) (error, int64) {
	// 根据id更新
	tx := dao.DB.Model(&Course{}).Where("id = ?", newCou.ID).Save(newCou)
	return tx.Error, tx.RowsAffected
}

func CourseDeleteDB(id int) (error, int64) {
	// 根据id硬删除
	tx := dao.DB.Unscoped().Delete(&Course{}, id)
	return tx.Error, tx.RowsAffected
}
